...
#		full.c <- full.c[,selected$covar.keep]
	...
	if (cv==FALSE){
		...
		min.c <- nloptr(x0 = as.matrix(rep(0, length(grouping)-1)),
                eval_f = crossValidate,
                lb = rep(0, length(grouping)-1),
                ub = rep(5, length(grouping)-1),
		...
		#opt_constraints <- c(0, 5)
#		min.c <- optim(par=as.matrix(rep(0, length(grouping)-1)),
#			crossValidate, 
#            grouping=grouping,
#            folds=folds,
#            treatment = X[Treatment==1,],
...
#			control = list(lmm = 15, factr = 1e-10, maxit = 500)
#			)

#		min.c <- nmkb(par=as.matrix(rep(0, length(grouping)-1)),
#			crossValidate, 
...
#			)
		...
		opt_constraints <- c(-10, 10)
#		z <- optim(par=as.matrix(rep(0, length(coefs))), 
#			loss_func0, 
#			co_x=co.x, 
#			tr_total=as.matrix(tr.total),
#			Q=base.weight,
#			alpha = as.matrix(rep(c(0, c(min.c$par)), times=grouping)),
#			method='L-BFGS-B',
#			lower = opt_constraints[1], 
#			upper = opt_constraints[2], 
#			control = list(lmm = 15, factr = 1e-10, maxit = 500)
...
#		z <- nmkb(par=as.matrix(rep(0, length(coefs))), 
#			loss_func0, 
#			control = list(tol=1e-4),
#			lower = opt_constraints[1], 
#			upper = opt_constraints[2], 
...
#			)
		...
		z <- nloptr(x0 = as.matrix(rep(0, length(coefs))), 
                eval_f = loss_func0,
                ...
                tr_total=as.matrix(tr.total),
                Q=base.weight,
                alpha = as.matrix(rep(c(0, min.c$solution), times=grouping)))
	} #end of if, else

	cc <- as.matrix(z$solution)
	weights <- c(entbal_wts(base.weight, co.x, cc))
